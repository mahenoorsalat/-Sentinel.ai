...
const LOGS = [
  ...
  { t: "14:22:09", tag: "THREAT", msg: "SQL injection vector @/users/find", tone: "danger" },
  {
    t: "14:22:14",
    tag: "REMEDIATOR",
    msg: "Patch synthesized — parameterized query",
    tone: "success",
  },
      ...
      <header className="sticky top-0 z-50 border-b border-border/60 bg-background/80 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
          ...
          <div className="flex items-center gap-3">
            <Link
              to="/console"
              className="inline-flex items-center gap-1.5 rounded-full bg-[var(--ink)] px-4 py-2 text-sm font-semibold text-[var(--ink-foreground)] shadow-soft transition-transform hover:scale-[1.02]"
            >
        ...
        <section className="grid grid-cols-12 gap-4 pt-10 pb-6">
          ...
          <div className="col-span-12 flex flex-col items-center text-center">
            ...
            <p className="mt-6 max-w-xl text-pretty text-base text-muted-foreground md:text-lg">
              Sentinel deploys a swarm of AI agents that audit your stack, detect vulnerabilities,
              and ship remediation in real time — without a SOC team.
            </p>
            <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
              <Link
                to="/console"
                className="inline-flex items-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground shadow-[0_8px_24px_-8px_oklch(0.52_0.22_280/0.5)] transition-transform hover:scale-[1.02]"
              >
                <Sparkles className="size-4" /> Start a deep scan
              </Link>
              <Link
                to="/console"
                className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-5 py-3 text-sm font-semibold text-foreground shadow-soft transition-colors hover:bg-secondary"
              >
            ...
            <div className="flex items-start justify-between">
              ...
              <div className="flex -space-x-2">
                {[0, 1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="size-8 rounded-full border-2 border-card bg-gradient-to-br from-primary/40 to-[var(--peach)]"
                  />
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-12 gap-0">
              ...
              <div className="relative col-span-12 min-h-[280px] border-b border-border bg-[radial-gradient(circle_at_1px_1px,oklch(0.85_0.02_280)_1px,transparent_0)] [background-size:22px_22px] p-6 md:col-span-7 md:border-b-0 md:border-r">
                ...
                <div className="grid h-full grid-cols-5 items-center">
                  {(["Input", "Orchestrator", "Scanner", "Remediator", "Report"] as const).map(
                    (label, i) => (
                      <div key={label} className="relative flex flex-col items-center gap-2">
                        <div
                          className={`relative grid size-12 place-items-center rounded-2xl border ${i === 2 ? "border-primary/40 bg-primary/10" : "border-border bg-card"} shadow-soft`}
                        >
                          ...
                          {i === 2 && (
                            <span className="absolute inset-0 animate-pulse-ring rounded-2xl border-2 border-primary/60" />
                          )}
                        </div>
                ...
                <div className="mt-3 flex-1 space-y-2.5 font-mono text-[11px]">
                  {LOGS.map((l, i) => {
                    ...
                    return (
                      <div
                        key={i}
                        className={`flex gap-2 transition-opacity ${visible ? "opacity-100" : "opacity-20"}`}
                        style={{ animation: visible ? "ticker .35s ease-out both" : undefined }}
                      >
          ...
          <div className="col-span-12 lg:col-span-4 flex flex-col gap-4">
            <div className="rounded-3xl border border-primary/20 bg-gradient-to-br from-[var(--lilac)] to-card p-5 shadow-card">
              ...
              <p className="mt-3 text-sm leading-relaxed text-foreground">
                Agent <span className="font-mono font-semibold text-primary">ALPHA-4</span> drafted
                a parameterized query patch for{" "}
                <code className="rounded bg-card px-1 py-0.5 text-[11px]">/api/users/find</code>.
              </p>
            ...
            <div className="rounded-3xl border border-border bg-[var(--peach)]/40 p-5 shadow-soft">
              ...
              <div className="mt-3 space-y-2">
                {[
                  ...
                  <div
                    key={v.label}
                    className="flex items-center justify-between rounded-xl bg-card/80 px-3 py-2 text-xs backdrop-blur"
                  >
                    ...
                    <span
                      className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${
                        v.tone === "danger"
                          ...
                          : v.tone === "warning"
                            ? "bg-[var(--warning)]/20 text-[oklch(0.45_0.14_70)]"
          ...
          <div className="grid grid-cols-12 gap-6">
            <div className="col-span-12 md:col-span-5">
              ...
              <p className="mt-4 max-w-md text-muted-foreground">
                Each agent owns a slice of the security loop. The orchestrator routes work, agents
                collaborate, results stream into your dashboard in real time.
              </p>
            </div>
              ...
              {[
                {
                  icon: Command,
                  name: "Orchestrator",
                  desc: "Routes tasks across the swarm with token-aware scheduling.",
                  color: "var(--lilac)",
                },
                {
                  icon: Zap,
                  name: "Scanner α",
                  desc: "Probes endpoints, headers, dependencies, and auth flows.",
                  color: "var(--mint)",
                },
                {
                  icon: Activity,
                  name: "Risk Analyst",
                  desc: "Scores severity, exploitability, and business impact.",
                  color: "var(--peach)",
                },
                {
                  icon: Sparkles,
                  name: "Remediator",
                  desc: "Synthesizes code patches and config fixes you can ship.",
                  color: "var(--lilac)",
                },
              ].map((a) => (
                <div
          ...
          <div className="relative overflow-hidden rounded-3xl border border-border bg-card p-8 shadow-card md:p-12">
            ...
            <div className="relative grid grid-cols-12 gap-8">
              <div className="col-span-12 md:col-span-6">
                ...
                <p className="mt-4 max-w-md text-muted-foreground">
                  Scan unlimited times. Only pay for the surface you actually monitor.
                </p>
              </div>
              <div className="col-span-12 md:col-span-6">
                <div className="rounded-2xl border border-border bg-background p-6 shadow-soft">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="rounded-full bg-[var(--mint)]/60 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-foreground">
                      14-day trial
                    </div>
                  </div>
                  <ul className="mt-5 space-y-2 text-sm text-muted-foreground">
                    <li className="flex items-center gap-2">
                      <ShieldCheck className="size-4 text-[var(--success)]" /> All 5 autonomous
                      agents
                    </li>
                    <li className="flex items-center gap-2">
                      <ShieldCheck className="size-4 text-[var(--success)]" /> Realtime remediation
                      patches
                    </li>
                    <li className="flex items-center gap-2">
                      <ShieldCheck className="size-4 text-[var(--success)]" /> SOC 2 audit reports
                    </li>
                  </ul>
